...
SELECT name,
	...
	ELSE 'cheap' END AS cost_judgement
  FROM Facilities;
...
SELECT 	DISTINCT CONCAT(members.firstname , ' ' , members.surname) AS member_name,
	...
	ELSE 'Tennis Court 2' END AS tennis_court_used
...
SELECT	facilities.name AS facility_name,
  ...
  WHERE starttime LIKE '2012-09-14%' AND ((bookings.memid = 0 AND facilities.guestcost * bookings.slots > 30) OR (bookings.memid > 0 AND facilities.membercost * bookings.slots > 30))
  ORDER BY total_cost DESC;

/* Q9: This time, produce the same result as in Q8, but using a subquery. */

SELECT	facilities_bookings.name AS facility_name,
	CONCAT(members.firstname, ' ', members.surname) AS member_name,
	facilities_bookings.total_cost
  ...
  JOIN (
	SELECT 	facilities.name,
    	...
    	LEFT JOIN Facilities facilities
    	ON bookings.facid = facilities.facid
	WHERE bookings.starttime LIKE '2012-09-14%') facilities_bookings
  ON members.memid = facilities_bookings.memid
  WHERE facilities_bookings.total_cost > 30
  ORDER BY total_cost DESC;
...
SELECT	sub.name,
	SUM(sub.slot_revenue) AS total_revenue
  FROM (
	SELECT 	facilities.facid,
    		facilities.name,
    		CASE WHEN bookings.memid = 0 THEN facilities.guestcost * bookings.slots
    		ELSE facilities.membercost * bookings.slots END AS slot_revenue
    	FROM Bookings bookings
    	LEFT JOIN Facilities facilities
